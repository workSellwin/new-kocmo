        ...
        <ul class="nav">
            <? foreach ($arResult as $arMenuL1) { ?>
                ...
                <li class="nav__item <?= $active ?>">
                    <? $cim = strpos($arMenuL1['LINK'], '?') === false ? '?' : '&' ?>
                    <? if (true or $arParams['SELECTED_INDEX'] !== $arMenuL1["ITEM_INDEX"]): ?>
                        <a href="<?= $arMenuL1['LINK'] ?><?= $cim ?>available_yes=y"
                           class="nav__lnk"><?= $arMenuL1['TEXT'] ?></a>
                    <? else: ?>
                        ...
                        <div class="nav-dropdown">
                            <div class="nav-dropdown__sub sub<?= $sub ?>">
                                ...
                                <? foreach ($arMenuL1['CHILD'] as $arMenuL2) { ?>
                                    ...
                                    if (true or $arParams['SELECTED_INDEX'] !== $arMenuL2['ITEM_INDEX']) {
                            ...
                            <div class="nav-dropdown__img  sub-img<?= $sub ?>">
                                <? if ($arMenuL1['INFO']['PICTURE']) { ?>
                                    <? $url = $arMenuL1['INFO']['URL']; ?>
                                    <? if ($url) { ?>
                                        <a class="view-a" href="<?= $url ?>"><img
                                                    src="<?= $arMenuL1['INFO']['PICTURE'] ?>"
                                                    alt="<?= $arMenuL1['TEXT'] ?>"></a>
                                    <? } else { ?>
