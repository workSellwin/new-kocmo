  ...
  if (TreeItems.length > 0){
    TreeItems.each(function(){
      if(jQuery.inArray(String(jQuery(this).data("id")), self.currentFavorites) >= 0){
        jQuery(this).addClass("products-item__wishlist--active");
      }else{
        jQuery(this).removeClass("products-item__wishlist--active");
      }
    });
  }
  ...
  jQuery(document).on('click', "."+this.button.className, function (e) {
    ...
    jQuery.ajax(link, {
      ...
      success: function (data) {
        if(data.ADD > 0){
          $this.addClass("products-item__wishlist--active");
        }
        if(data.DELETE > 0){
          $this.removeClass("products-item__wishlist--active");
        }
