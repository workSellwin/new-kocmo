...
<div class="afbf alx_feed_back <? $arParams['INPUT_APPEARENCE'] != 'DEFAULT' ? strtolower($arParams['INPUT_APPEARENCE']) : '' ?>"
    ...
    <? else: ?>
    <div style="width: 100%"
         class="afbf alx_feed_back <? foreach ($arParams['INPUT_APPEARENCE'] as $param) if ($param != 'DEFAULT') echo strtolower($param) . ' ' ?>"
         id="alx_feed_back_<?= $ALX ?>">
        <? endif ?>
        <?
        ?>
    ...
    <?
        ...
        <div class="afbf_success_block<? if ($arParams['ALX_LINK_POPUP'] !== 'Y'): ?> _without-popup<? endif; ?> container">
    ...
    <? endif; ?>
        <script type="text/javascript">
            ...
            function AltasibFeedbackRedirect_<?=$ALX?>(ORDER_ID) {
                document.location.href = '<?=(trim(htmlspecialcharsEx($arParams['LOCAL_REDIRECT_URL'])));?>'+ORDER_ID;
            }

                <?if($arResult['EGIFT_ELEMENT_ID']):?>
                    var ID = <?=$arResult['EGIFT_ELEMENT_ID']?>;

                        $.post(
                            "/ajax/index.php",
                            {
                                ACTION: "egift",
                                METHOD: "creatNewOrder",
                                PARAMS: {
                                    'EGIFT_ELEMENT_ID': ID,
                                },
                            },
                        ...
                        function onAjaxSuccess(response) {
                            if (response) {
                               AltasibFeedbackRedirect_<?=$ALX?>(response);
                            }
                        }
                <?endif;?>
