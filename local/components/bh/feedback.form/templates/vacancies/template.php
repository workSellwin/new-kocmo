...
<? if ($arParams['ALX_LINK_POPUP'] !== 'Y'): ?>
    ...
    <div id="afbf_err_<?= $ALX ?>" style="width: 100%;" class="alx-feedb-error"></div>
<? endif ?>
<?
...
<div style="width: 100%" class="afbf alx_feed_back <? $arParams['INPUT_APPEARENCE'] != 'DEFAULT' ? strtolower($arParams['INPUT_APPEARENCE']) : '' ?>"
     id="alx_feed_back_<?= $ALX ?>">
    <? else: ?>
    <div style="width: 100%" class="afbf alx_feed_back <? foreach ($arParams['INPUT_APPEARENCE'] as $param) if ($param != 'DEFAULT') echo strtolower($param) . ' ' ?>"
         id="alx_feed_back_<?= $ALX ?>">
        <? endif ?>
        <?
        ?>
    ...
    <?
        ...
        <div class="afbf_success_block<? if ($arParams['ALX_LINK_POPUP'] !== 'Y'): ?> _without-popup<? endif; ?>">
            <div class="afbf_mess_ok">
                <div style="text-align: center;" class="mess">Спасибо! Ваша заявка успешно отправлена</div>
            </div>
        </div>
    <? if ($arParams['ALX_LINK_POPUP'] == 'Y'): ?>
    ...
    <? if ($arParams['SHOW_LINK_TO_SEND_MORE'] == 'Y'): ?>
        <div class="afbf_send_another_message">
            <a class="popup-vacancies__form-submit btn" href="<?= $APPLICATION->GetCurUri() ?>">Закрыть</a>
        </div>
    <? endif; ?>
        <script type="text/javascript">
