...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["FEEDBACK_FORM_" . $ALX] && $arParams["SECTION_FIELDS_ENABLE"] == "Y" && $_POST["REFRESH"] == "Y") {
    ...
    if (count($arResult["FORM_ERRORS"]) <= 0) {
        ...
        if ($arParams['EGIFT'] == 'Y') {
            $arElementFields['XML_ID']=$_REQUEST['FIELDS']['GUI_SERTIFIKAT_FIDFORM_SERTIFIKAT'];
            $arElementFields['NAME'] .= ' на сумму ' . $_REQUEST['FIELDS']['SUM_SERTIFIKAT_FIDFORM_SERTIFIKAT'] . ' руб.';
        }
